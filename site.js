...
function renderFigureSlider(data, ids) {
  var slider = document.getElementById(ids.slider);
  if (!slider || !data || !data.length) return;

  var items = shuffled(data);
  var current = 0;
  var timer = null;

  slider.innerHTML = items.map(function (f, i) {
    return '<a href="' + f.url + '" target="_blank" rel="noopener" class="figure-slide' + (i === 0 ? ' active' : '') + '">' +
        '<div class="figure-slide-img"><img src="' + f.image + '" alt="' + escapeHtml(f.title) + '" loading="lazy" /></div>' +
        '<div class="figure-slide-info">' +
          '<p class="text-xs font-bold uppercase tracking-widest accent-text">' + f.year + '</p>' +
          '<h3 class="font-serif font-bold text-xl mt-2 leading-snug">' + escapeHtml(f.title) + '</h3>' +
          '<p class="text-sm text-[#4D4F53] mt-3 leading-relaxed">' + escapeHtml(f.description) + '</p>' +
          '<span class="inline-block mt-4 text-xs font-bold uppercase tracking-widest accent-text">Read the paper →</span>' +
        '</div>' +
      '</a>';
  }).join('');
  ...
  if (dotsWrap) {
    dotsWrap.innerHTML = items.map(function (_, i) {
      return '<button class="figure-dot' + (i === 0 ? ' active' : '') + '" aria-label="Go to figure ' + (i + 1) + ' of ' + items.length + '"></button>';
    }).join('');
  }

  var slides = [].slice.call(slider.querySelectorAll('.figure-slide'));
  ...
  dots.forEach(function (d, i) { d.addEventListener('click', function () { show(i); restartTimer(); }); });
...
function renderFigures() {
  renderFigureSlider(typeof FIGURES === 'undefined' ? null : FIGURES, { slider: 'figure-slider', prev: 'figure-prev', next: 'figure-next', dots: 'figure-dots' });
